(function() {
  ...
  // we roll our own "linked list" because we don't need very many features
  var Linked = (function() {
    ...
    Linked.prototype.each = function(fn) {
      for (var item = this._head; item; item = item.next)
        fn(item.data);
    };

    Linked.prototype.array = function() {
      var array = new Array(this.length);
      for (var i = 0, item = this._head; item; item = item.next)
        array[i++] = item.data;
      return array;
    };
  ...
  var nicely = function nicely(callback) {
    ...
    defer(function begin() {
      ...
      tasks.each(function(task) {
        task.fn.apply(tasks.me, tasks.args);
      });
    });
    ...
    return function run(field, fn) {
      ...
      // add to active tasks
      tasks.add(new Task(fn, this, args));
    };
  };
